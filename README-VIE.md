...
25. ### How Virtual DOM works?
    ...
    1. Khi data thay đổi, toàn bộ UI trong Virtual DOM sẽ re-render.

        ![vdom](images/vdom1.png)

    2. So sánh Virtual DOM và Real DOM để tìm ra sự khác nhau.

        ![vdom2](images/vdom2.png)

    3. Real DOM sẽ chỉ cập nhật những phần đã thay đổi so với DOM cũ được tính toán ở bước trên.
...
26. ### What is the difference between Shadow DOM and Virtual DOM?
    ...
    *Shadow DOM* là một công nghệ của trình duyệt dành cho scoping variables và CSS trong *web components*.

    *Virtual DOM* là khái niệm được các libs triển khai sử dụng JavaScript trên các API của trình duyệt
...
30. ### What are uncontrolled components?
    ...
    **Uncontrolled Component** là các component mà lưu trữ state một cách nội bộ, và nếu cần lấy các value của component thì bạn phải dùng ref để truy cập vào DOM. Nó gần giống như HTML truyền thống.
    ...
    Trong hầu hết các trường hợp, React đều khuyên dùng controlled components.
...
31. ### What is the difference between createElement and cloneElement?
    ...
    Hàm `cloneElement()` được dùng để clone một element và truyền cho nó các props mới.
...
33. ### What are the different phases of component lifecycle?
    ...
    1. **Mounting:** Component sẵn sàng để mount trong browser DOM. Phase này bao gồm các quá trình khởi tạo từ các phương thức `constructor()`, `getDerivedStateFromProps()`, `render()`, và `componentDidMount()`.

    2. **Updating:** Trong phase này, component cập nhật theo hai cách, gửi các props mới và update state từ hai hàm `setState()` hoặc `forceUpdate()`. Phase này bao gồm các phương thức `getDerivedStateFromProps()`, `shouldComponentUpdate()`, `render()`, `getSnapshotBeforeUpdate()` và `componentDidUpdate()`.
    
    3. **Unmounting:** Trong phase này, component sẽ unmount từ browser DOM. Phase bao gồm phương thức `componentWillUnmount()`.

    Điều đáng nói là React hình thành các khái niệm này dựa trên quá trình thực thi các thay đổi tới DOM. Chúng được chia ra như sau: 
    
    1. **Render** Component sẽ được render mà không có side-effects. Áp dụng cho pure component và trong phase này, React có thể pause, abort hoặc restart render.

    2. **Pre-commit** Trước khi component thực sự áp dụng các thay đổi vào DOM, sẽ có một khoảng thời gian cho phép React đọc DOM thông qua hàm `getSnapshotBeforeUpdate()`.

    3. **Commit** React hoạt động với DOM và thực thi các lifecycle cuối cùng là `componentDidMount()` để mounting, `componentDidUpdate()` để updating, và `componentWillUnmount()` để unmounting.

    React 16.3+ Phases (hoặc [bản interactive](http://projects.wojtekmaj.pl/react-lifecycle-methods-diagram/))
...
34. ### What are the lifecycle methods of React?
    ...
    - **componentWillMount:** Thực thi trước khi render và được dùng cho App level configuration trong root component.
    - **componentDidMount:** Thực thi sau lần render đầu tiên. Tại đây tất cả các AJAX requests, DOM hoặc state updates, và set up event listeners được chạy.
    - **componentWillReceiveProps:** Thực thi khi một phần prop update để kích hoạt việc chuyển đổi state.
    - **shouldComponentUpdate:** Xác định component có được update hay không. Mặc định thì nó return `true`. Nếu chắc chắn component không cần render sau khi state hoặc props update thì return `false`. Mục đích của phương thức này nhằm cải thiện hiệu năng vì nó cho phép chúng ta ngăn chặn việc component re-render khi nhận được prop mới.
    - **componentWillUpdate:** Thực thi trước khi component re-render khi có props hoặc state thay đổi, chỉ khi `shouldComponentUpdate()` return true.
    - **componentDidUpdate:** Hầu như được dùng để update DOM trong việc phản hồi khi state hoặc prop thay đổi. 
    - **componentWillUnmount:** Được sử dụng để cancel các network requests gửi đi, hoặc remove tất cả event listeners liên quan tới component đó.

    React 16.3+

    - **getDerivedStateFromProps:** Được gọi ngay trước hàm `render()` và *luôn* được gọi khi component render. Dùng trong các trường hợp cần derived state (hiếm khi dùng). Đọc thêm [nếu bạn cần derived state](https://reactjs.org/blog/2018/06/07/you-probably-dont-need-derived-state.html).
     - **componentDidMount:** Thực thi sau lần render đầu tiên. Tại đây tất cả các AJAX requests, DOM hoặc state updates, và set up event listeners được chạy.
    - **shouldComponentUpdate:** Xác định component có được update hay không. Mặc định thì nó return `true`. Nếu chắc chắn component không cần render sau khi state hoặc props update thì return `false`. Mục đích của phương thức này nhằm cải thiện hiệu năng vì nó cho phép chúng ta ngăn chặn việc component re-render khi nhận được prop mới.
    - **getSnapshotBeforeUpdate:** Thực thi ngay sau khi các output đã được render và được committed vào DOM. Bất kì giá trị nào được return từ phương thức này sẽ được truyền vào `componentDidUpdate()`. Điều này hữu ích cho việc bắt thông tin từ DOM cụ thể như scroll position.
    - **componentDidUpdate:** Hầu như được dùng để update DOM trong việc phản hồi khi state hoặc prop thay đổi. Phương thức này không chạy nếu `shouldComponentUpdate()` returns `false`.
    - **componentWillUnmount:** Được sử dụng để cancel các network requests gửi đi, hoặc remove tất cả event listeners liên quan tới component đó.
...
35. ### What are Higher-Order components?
    ...
    Chúng ta gọi nó là **pure component** vì nó có thể accept bất kỳ các component con nào được cung cấp động nhưng nó không chỉnh sửa hoặc copy bất kỳ behavior nào từ component đầu vào.

    ```javascript
    const EnhancedComponent = higherOrderComponent(WrappedComponent)
    ```

    HOC được dùng trong các trường hợp:

    1. Code reuse, logic và bootstrap abstraction.
    2. Render hijacking.
    3. State abstraction và manipulation.
    4. Thao tác với props.
...
37. ### What is context?
    ...
    *Context* cung cấp một cách để truyền data qua component tree mà không cần truyền bằng props xuống dần từng level.
    ...
    Ví dụ, authenticated user, locale preference, UI theme cần truy cập vào ứng dụng bằng nhiều components.
...
38. ### What is children prop?
    ...
    *Children* là một prop (`this.props.children`) cho phép bạn truyền các component như prop data vào các component khác. Component tree được đặt giữa hai tag của component sẽ được truyền tới component đó như là một `children` prop.
    ...
    Có một số lượng lớn các phương thức có sẵn trong React API hoạt động với prop kiểu này. Bao gồm `React.Children.map`, `React.Children.forEach`, `React.Children.count`, `React.Children.only`, `React.Children.toArray`.

    Dưới đây là một cách sử dụng children prop đơn giản,
...
40. ### What is the purpose of using super constructor with props argument?
    ...
    **Not passing props:**
    ...
    Đoạn code trên chỉ ra rằng `this.props` chỉ khác nhau trong hàm contructor. Còn ngoài constructor thì nó không khác biệt.
...
43. ### What would be the common mistake of function being called every time the component renders?
    ...
    ```
    Thay vì vậy, ta cần truyền hàm dưới dạng tham số, bỏ dấu ngoặc đơn:    
...
44. ### Is lazy function supports named exports?
    ...
    ```
    và reexport `MoreComponents.js` components trong file trung gian `IntermediateComponent.js`
    ```javascript
    // IntermediateComponent.js
    export { SomeComponent as default } from "./MoreComponents.js";
    ```
    Bây giờ đã có thể import module sử dụng lazy function như dưới đây,
    ```javascript
...
46. ### What are fragments?
    ...
    ```
    Cũng có một *shorter syntax*, nhưng không được support trong nhiều tools:
...
47. ### Why fragments are better than container divs?
    ...
    2. Một vài cơ chế CSS như *Flexbox* và *CSS Grid* có một mối quan hệ parent-child đặc biêt, và thêm các thẻ div và giữa sẽ khiến chúng ta khó giữ được layout như mong muốn.
    3. Trình kiểm tra DOM sẽ ít bị xáo trộn.
...
48. ### What are portals in React?
    ...
    *Portal* là một cách được khuyến nghị để render children thành một DOM node tồn tại bên ngoài DOM hierarchy của component cha.
    ...
    Đối số đầu tiên là bất kỳ React child nào có thể render, như một element, string, hoặc một fragment. Đối số thứ hai là một DOM element.    
...
49. ### What are stateless components?
    ...
    Stateless component là component mà behaviour của nó độc lập với state. Bạn có thể sử dụng function hoặc class để tạo stateless component. Bạn nên dùng function component trừ khi bạn cần lifecycle hook. Function component có rất nhiều lợi ích: dễ viết, hiểu và test, nhanh hơn một ít và có thể tránh việc sử dụng keyword `this`.
...
50. ### What are stateful components?
    ...
    Stateful component là component mà behaviour của nó phụ thuộc vào *state*. *Stateful components* luôn là *class components* và có state khởi tạo trong `constructor`. 
    ...
    **React 16.8 Update:**
    ...
    Hooks giúp chúng ta sử dụng state và các tính năng của React khác mà không cần phải viết class.
...
51. ### How to apply validation on props in React?
    ...
    Tập các prop types được xác định trước:
    ...
    Chúng ta có thể định nghĩa `propTypes` cho `User` component như dưới đây:
    ...
    **Note:** Trong React v15.5 *PropTypes* được chuyển từ `React.PropTypes` tới thư viện `prop-types`.
...
52. ### What are the limitations of React?
    ...
    2. JSX dễ đọc hiểu.
    3. Nó render cả ở client và server side (*SSR*).
    4. Dễ tích hợp với các frameworks khác (Angular, Backbone) vì nó chỉ là một view library.
    5. Dễ viết unit và integration tests bằng các tools như Jest.
...
53. ### What are the limitations of React?
    ...
    2. Lộ trình học ngoằn ngoèo cho người mới.
    3. Việc tích hơp React vào các MVC framework truyền thống yêu cầu config thêm một số thứ.
    4. Độ phức tạp của code tăng vì các template inline và JSX.
    5. Quá nhiều component nhỏ dẫn tới khó xử lý.
...
54. ### What are error boundaries in React v16?
    ...
    *Error boundaries* là các component catch JavaScript errors ở bất kỳ chỗ nào trong child component tree, log các errors đó ra, và hiển thị một UI dự phòng thay vì để cả component tree crash.
    ...
    Một class component trở thành error boundary nếu nó có định nghĩa lifecycle  `componentDidCatch(error, info)` hoặc `static getDerivedStateFromError() `:
    ...
    Sau đó ta có thể sử dụng nó như một component bình thường:
...
58. ### What is the purpose of render method of react-dom?
    ...
    Nếu có thêm tham số callback, callback này sẽ thực thi ngay sau khi component render hoặc update.
...
60. ### How to use InnerHtml in React?
    ...
    Trong ví dụ này MyComponent dùng thuộc tính `dangerouslySetInnerHTML` để setting HTML markup:
...
61. ### How to use styles in React?
    ...
    Style keys là camelCased để thích hợp cho việc truy cập các thuộc tính này từ DOM nodes trong JavaScript (e.g. `node.style.backgroundImage`)
...
62. ### How events are different in React?
    ...
    1. React event handlers đặt tên theo camelCase, thay vì lowercase.
    2. Với JSX bạn có thể truyền một function như một event handler, thay vì một string như thông thường.
...
64. ### What is the impact of indexes as keys?
    ...
    Trong đoạn code dưới đây, mỗi key dựa trên thứ tự của element, thay vì gắn với data của element đó. Điều này hạn chế những tối ưu hóa của React.
    ...
    Giả sử chúng ta gán todo.id vào key, với todo.id là độc nhất và bất biến, React sẽ có thể sắp xếp lại element mà không cần tính toán lại nhiều.
...
70. ### How do you memoize a component?
    ...
    Ví dụ, `moize` library có thể memoize component trong một component khác.
    ...
    **Update:** Từ React v16.6.0 trở đi, chúng ta có khái niệm `React.memo`. Nó cung cấp một higher order component để ghi nhớ component trừ khi props thay đổi. Dùng React.memo bọc xung quanh component trước khi sử dụng nó.
...
73. ### What is CRA and its benefits?
    ...
    Các bước tạo Todo App sử dụng *CRA*:
    ...
    $ npm start
    ```
    Nó bao gồm mọi thứ cần thiết để build một React app:

    1. React, JSX, ES6, và Flow.
    2. Ngôn ngữ bổ sung ngoài ES6 như object spread operator.
    3. Autoprefixed CSS, do đó không cần dùng -webkit- hoặc các prefixes khác.
    4. Trình chạy unit test tương tác nhanh với built-in hỗ trợ cho việc coverage reporting.
    5. Live development server cảnh báo các lỗi thông thường.
    6. Một build script nhằm đóng gói JS, CSS, và hình ảnh thành sản phẩm sử dụng hashes và sourcemaps.
...
82. ### Why we need to pass a function to setState()?
    ...
    `setState()` là hàm async, state không thay đổi ngay lập tức sau khi gọi hàm `setstate()`. Do đó bạn không thể chắc chắn state hiện tại là gì ngay sau khi `setState()` được gọi. Giải pháp là chúng ta sẽ truyền một function vào `setState()`, với đối số chính là state trước đó.
...
83. ### What is strict mode in React?
    ...
    `React.StrictMode` là component hữu ích để highlight các lỗi trong app. Ví dụ như `<Fragment>`, `<StrictMode>` sẽ không render thêm bất kỳ DOM elements nào. Nó sẽ check và cảnh báo cho tất cả các component bên trong. Chỉ áp dụng cho chế độ *development mode*.
    ...
    Trong ví dụ trên, *strict mode* checks chỉ áp dụng cho `<ComponentOne>` và `<ComponentTwo>`.
...
87. ### Why should component names start with capital letter?
    ...
    Nếu chúng ta render các component sử dụng cú pháp JSX, tên của các component đó sẽ phải viết hoa chữ cái đầu nếu không thì React sẽ throw ra lỗi unrecognized tag. Bởi vì chỉ HTML elements và SVG tags mới bắt đầu bằng chữ cái thường.
    ...
    ```
    Bạn có thể đặt tên Component bắt đầu bằng chữ thường, nhưng nếu import component này vào file khác thì cần phải đặt lại tên bắt đầu bằng chữ hoa cho nó. Dưới đây ta khởi tạo component bắt đầu bằng chữ thường
    ...
    Khi import vào file khác thì nó phải bắt đầu bằng chữ hoa:
    ...
    Tên component nên được bắt đầu bằng chữ hoa nhưng có một vài trường hợp ngoại lệ. Tag name có thể dùng chữ thường với dấu chấm ngăn cách (property accessors).
    Tag dưới đây vẫn được xem là hợp lệ:
